...
export function EventCard({ event, occursAt }: { event: EventDoc; occursAt?: string }) {
  ...
  return (
    <Link href={`/events/${event.slug}`}
      className="block min-w-0 rounded-r-lg border-l-2 border-dotted bg-[var(--surface)] px-4 py-3 ring-1 ring-[var(--line-soft)] transition-colors hover:bg-[var(--surface-2)]"
